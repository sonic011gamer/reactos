                 ...
                 OUT PVOID *Object)
{
    ...
    *KeyCell = HvAllocateCell(Hive,
    ...
    if (NT_SUCCESS(Status))
    {
         ...
         * CmpAssignSecurityDescriptor function does not do anything
 ...
 * by this function to the caller. If no subkey levels is found
 * the function returns 0.
 *
 * @return
 * Returns the number of remaining subkey levels to caller.
 * If no subkey levels is found then this function returns 0.
 */
ULONG
NTAPI
CmpComputeHashValue(
    ...
    _Out_ PULONG TotalSubKeys)
{
    ...
    while (*RemainingNameBuffer == OBJ_NAME_PATH_SEPARATOR)
    {
        RemainingNameBuffer++;
    ...
    {
        /* Is this character a slash? */
        if (*RemainingNameBuffer != OBJ_NAME_PATH_SEPARATOR)
        {
            /* It's not, add it to the hash */
            CopyConvKey = 37 * CopyConvKey + RtlUpcaseUnicodeChar(*RemainingNameBuffer);
        ...
        {
            /* We have a leading slash here, skip it */
            while (*RemainingNameBuffer == OBJ_NAME_PATH_SEPARATOR)
            {
                RemainingNameBuffer++;
                RemainingNameLength -= sizeof(WCHAR);
            ...
            if (RemainingSubkeysInTotal < CMP_SUBKEY_LEVELS_DEPTH_LIMIT)
            {
                 ...
                 * to hold up the new subkey which we will compute
                 * the hashes for it, so that we don't blow up the
                 * hash stack.
                 */
                if (RemainingSubkeysInTotal < CMP_SUBKEY_LEVELS_DEPTH_LIMIT)
                {
...
/**
 * @brief
 * Compares each subkey's hash and name with ones
    ...
    _Inout_ PUNICODE_STRING RemainingName)
{
    ...
    /* Skip the leading slash on matching name */
    if (RemainingName->Length > 0)
    {
        while (RemainingName->Buffer[0] == OBJ_NAME_PATH_SEPARATOR)
        {
            RemainingName->Buffer++;
            RemainingName->Length -= sizeof(WCHAR);
        }
