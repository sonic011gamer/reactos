...
typedef UINT64 GfxAddress;    // Address in Gfx Virtual space
...
typedef struct ColorCalcState
{
    ...
    {
        float floatVal;
        ULONG intVal;         // ref value stored in high byte
    } alphaRef;
    float constR;
    float constG;
    float constB;
...
typedef struct SamplerState
{
    ULONG flags0;
    ULONG flags1;
    ULONG borderColorAddr;                // SAMPLER_BORDER_COLOR_STATE relative to Dynamic State Base Address
    ULONG flags2;
} SamplerState;
...
{
    ULONG drdy;            // Driver Ready
    ULONG csts;            // STATUS
    ULONG cevt;            // Current Event
    UCHAR  rsv2[0x14];
    ULONG didl[8];         // Supported Display Devices ID List (_DOD)
    ULONG cpdl[8];         // Current Attached (or Present) Display Devices List
    ULONG cadl[8];         // Current Active Display DevicesLists (_DCS)
    ULONG nadl[8];         // Next Active Devices List (_DGS use)
    ULONG aslp;            // ASL Sleep Time Out
    ULONG tidx;            // Toggle Table Index
    ULONG chpd;            // Current Hotplug Enable Indicator
    ULONG clid;            // Current Lid State Indicator
    ULONG cdck;            // Current Docking State Indicator
    ULONG sxsw;            // Request ASL to issue Display Switch notification on Sx State resume
    ULONG evts;            // Events Supported by ASL
    ULONG cnot;            // Current OS Notification
    ULONG nrdy;            // Driver Status
    UCHAR  rsv3[0x40];
...
{
    ULONG scic;             // SWSCI Command/Status/Data
    ULONG parm;             // Parameters
    ULONG dslp;             // Driver Sleep Time Out
    UCHAR  rsv4[0xF4];
...
{
    ULONG ardy;             // Driver Readiness
    ULONG aslc;             // ASLE Interrupt Command/Status
    ULONG tche;             // Technology Enable Indicator
    ULONG alsi;             // Current ALS Luminance Reading (in Lux)
    ULONG bclp;             // Requested Backlight Brightness
    ULONG pfit;             // Panel Fitting State or Request
    ULONG cblv;             // Current Brightness Level
    UINT16 bclm[20];         // Backlight Brightness Levels Duty Cycle Mapping Table
    ULONG cpfm;             // Current Panel Fitting Mode
    ULONG epfm;             // Enabled Panel Fitting Modes
    struct PLUT
    {
        UCHAR lutHeader;     // MWDD FIX: Prob need to break down to bits
        struct PanelId
        {
            UINT16 manufacturingId;
            UINT16 productId;
            ULONG serialNumbers;
            UCHAR  weekOfManufacture;
            UCHAR  yearOfManufacture;
        } panelId;
        UCHAR lutTable[7][9]; // MWDD FIX: Do I have rows/cols backwards?
    } plut;               // Panel LUT & Identifer
    ULONG pfmb;             // PWM Frequency and Minimum Brightness
    ULONG ccdv;             // Color Correction Default Values
    UCHAR  rsv4[0xF4];
