...
typedef /* int64_t */ LONG64 timeout_t;
#define TIMEOUT_INFINITE (((timeout_t)0x7fffffff) << 32 | 0xffffffff)

NTSTATUS  WINAPI RtlSleepConditionVariableCS(RTL_CONDITION_VARIABLE*,RTL_CRITICAL_SECTION*,const LARGE_INTEGER*);
...
#define GetProcessHeap()       (NtCurrentTeb()->ProcessEnvironmentBlock->ProcessHeap)
...
static ULONG CALLBACK threadpool_worker_proc( void *param );
 ...
 */
static ULONG CALLBACK timerqueue_thread_proc( void *param )
{
    ...
    DPRINT( "starting timer queue thread\n" );
    ...
    DPRINT( "terminating timer queue thread\n" );
    RtlExitUserThread( 0 );
    return 0;
}
 ...
 */
static ULONG CALLBACK waitqueue_thread_proc( void *param )
{
    ...
    DPRINT( "starting wait queue thread\n" );
        ...
        else
        {
            ...
            if (status >= STATUS_WAIT_0 && status < STATUS_WAIT_0 + num_handles)
            {
                ...
                if (wait->u.wait.bucket)
                {
                    ...
                    DPRINT1("wait object %p triggered while object was destroyed\n", wait);
            }
    ...
    DPRINT( "terminating wait queue thread\n" );
...
static BOOL tp_threadpool_release( struct threadpool *pool )
{
    ...
    DPRINT( "destroying threadpool %p\n", pool );
                                  ...
                                  PVOID userdata, TP_CALLBACK_ENVIRON *environment )
{
    ...
    if (environment)
    {
        if (environment->Version != 1 && environment->Version != 3)
            DPRINT1( "unsupported environment version %u\n", environment->Version );
        ...
        if (environment->ActivationContext)
            DPRINT1( "activation context not supported yet\n" );

        if (environment->u.s.Persistent)
            DPRINT1( "persistent threads not supported yet\n" );
    }

    if (object->race_dll)
        LdrAddRefDll( 0, object->race_dll );

    DPRINT( "allocated object %p of type %u\n", object, object->type );
...
static BOOL tp_object_release( struct threadpool_object *object )
{
    ...
    DPRINT( "destroying object %p of type %u\n", object, object->type );
...
static ULONG CALLBACK threadpool_worker_proc( void *param )
{
    ...
    DPRINT( "starting worker thread for pool %p\n", pool );
    ...
    for (;;)
    {
        while ((ptr = list_head( &pool->pool )))
        {
            ...
            switch (object->type)
            {
                ...
                {
                    DPRINT( "executing simple callback %p(%p, %p)\n",
                           object->u.simple.callback, callback_instance, object->userdata );
                    object->u.simple.callback( callback_instance, object->userdata );
                    DPRINT( "callback %p returned\n", object->u.simple.callback );
                    break;
                }

                case TP_OBJECT_TYPE_WORK:
                {
                    DPRINT( "executing work callback %p(%p, %p, %p)\n",
                ...
                {
                    DPRINT( "executing timer callback %p(%p, %p, %p)\n",
                ...
                {
                    DPRINT( "executing wait callback %p(%p, %p, %p, %u)\n",
            ...
            {
                DPRINT( "executing finalization callback %p(%p, %p)\n",
    ...
    DPRINT( "terminating worker thread for pool %p\n", pool );
    tp_threadpool_release( pool );
    RtlExitUserThread( 0 );
    return 0;
}
 ...
 */
NTSTATUS NTAPI TpAllocCleanupGroup( TP_CLEANUP_GROUP **out )
{
    DPRINT( "%p\n", out );
 ...
 *           TpAllocPool    (NTDLL.@)
 */
NTSTATUS NTAPI TpAllocPool( TP_POOL **out, PVOID reserved )
{
    DPRINT( "%p %p\n", out, reserved );

    if (reserved)
        DPRINT1( "reserved argument is nonzero (%p)\n", reserved );
 ...
 *           TpAllocTimer    (NTDLL.@)
 */
NTSTATUS NTAPI TpAllocTimer( TP_TIMER **out, PTP_TIMER_CALLBACK callback, PVOID userdata,
 ...
 *           TpAllocWait     (NTDLL.@)
 */
NTSTATUS NTAPI TpAllocWait( TP_WAIT **out, PTP_WAIT_CALLBACK callback, PVOID userdata,
                             TP_CALLBACK_ENVIRON *environment )
{
    struct threadpool_object *object;
    struct threadpool *pool;
 ...
 *           TpAllocWork    (NTDLL.@)
 */
NTSTATUS NTAPI TpAllocWork( TP_WORK **out, PTP_WORK_CALLBACK callback, PVOID userdata,
                             TP_CALLBACK_ENVIRON *environment )
{
    struct threadpool_object *object;
    struct threadpool *pool;
 ...
 */
VOID NTAPI TpCallbackLeaveCriticalSectionOnCompletion( TP_CALLBACK_INSTANCE *instance, RTL_CRITICAL_SECTION *crit )
{
 ...
 */
NTSTATUS NTAPI TpCallbackMayRunLong( TP_CALLBACK_INSTANCE *instance )
{
    struct threadpool_instance *this = impl_from_TP_CALLBACK_INSTANCE( instance );
    ...
    {
        DPRINT1("called from wrong thread, ignoring\n");
 ...
 */
VOID NTAPI TpCallbackReleaseMutexOnCompletion( TP_CALLBACK_INSTANCE *instance, HANDLE mutex )
{
    struct threadpool_instance *this = impl_from_TP_CALLBACK_INSTANCE( instance );
 ...
 */
VOID NTAPI TpCallbackReleaseSemaphoreOnCompletion( TP_CALLBACK_INSTANCE *instance, HANDLE semaphore, DWORD count )
{
    struct threadpool_instance *this = impl_from_TP_CALLBACK_INSTANCE( instance );
 ...
 */
VOID NTAPI TpCallbackSetEventOnCompletion( TP_CALLBACK_INSTANCE *instance, HANDLE event )
{
    struct threadpool_instance *this = impl_from_TP_CALLBACK_INSTANCE( instance );
 ...
 */
VOID NTAPI TpCallbackUnloadDllOnCompletion( TP_CALLBACK_INSTANCE *instance, HMODULE module )
{
    struct threadpool_instance *this = impl_from_TP_CALLBACK_INSTANCE( instance );
 ...
 */
VOID NTAPI TpDisassociateCallback( TP_CALLBACK_INSTANCE *instance )
{
    struct threadpool_instance *this = impl_from_TP_CALLBACK_INSTANCE( instance );
    struct threadpool_object *object = this->object;
    struct threadpool *pool;
 ...
 *           TpIsTimerSet    (NTDLL.@)
 */
BOOL NTAPI TpIsTimerSet( TP_TIMER *timer )
{
 ...
 *           TpPostWork    (NTDLL.@)
 */
VOID NTAPI TpPostWork( TP_WORK *work )
{
 ...
 */
VOID NTAPI TpReleaseCleanupGroup( TP_CLEANUP_GROUP *group )
{
 ...
 */
VOID NTAPI TpReleaseCleanupGroupMembers( TP_CLEANUP_GROUP *group, BOOL cancel_pending, PVOID userdata )
{
    struct threadpool_group *this = impl_from_TP_CLEANUP_GROUP( group );
    ...
    LIST_FOR_EACH_ENTRY_SAFE( object, next, &members, struct threadpool_object, group_entry )
    {
        ...
        if (!object->shutdown)
        {
            ...
            {
                DPRINT( "executing group cancel callback %p(%p, %p)\n",
 ...
 *           TpReleasePool    (NTDLL.@)
 */
VOID NTAPI TpReleasePool( TP_POOL *pool )
{
 ...
 */
VOID NTAPI TpReleaseTimer( TP_TIMER *timer )
{
    struct threadpool_object *this = impl_from_TP_TIMER( timer );
 ...
 *           TpReleaseWait    (NTDLL.@)
 */
VOID NTAPI TpReleaseWait( TP_WAIT *wait )
{
 ...
 *           TpReleaseWork    (NTDLL.@)
 */
VOID NTAPI TpReleaseWork( TP_WORK *work )
{
    struct threadpool_object *this = impl_from_TP_WORK( work );
 ...
 */
VOID NTAPI TpSetPoolMaxThreads( TP_POOL *pool, DWORD maximum )
{
    struct threadpool *this = impl_from_TP_POOL( pool );
 ...
 */
BOOL NTAPI TpSetPoolMinThreads( TP_POOL *pool, DWORD minimum )
{
    struct threadpool *this = impl_from_TP_POOL( pool );
    NTSTATUS status = STATUS_SUCCESS;
 ...
 *           TpSetTimer    (NTDLL.@)
 */
VOID NTAPI TpSetTimer( TP_TIMER *timer, LARGE_INTEGER *timeout, LONG period, LONG window_length )
{
    struct threadpool_object *this = impl_from_TP_TIMER( timer );
    struct threadpool_object *other_timer;
 ...
 *           TpSetWait    (NTDLL.@)
 */
VOID NTAPI TpSetWait( TP_WAIT *wait, HANDLE handle, LARGE_INTEGER *timeout )
{
    struct threadpool_object *this = impl_from_TP_WAIT( wait );
 ...
 */
NTSTATUS NTAPI TpSimpleTryPost( PTP_SIMPLE_CALLBACK callback, PVOID userdata,
                                 TP_CALLBACK_ENVIRON *environment )
{
    struct threadpool_object *object;
    struct threadpool *pool;
 ...
 */
VOID NTAPI TpWaitForTimer( TP_TIMER *timer, BOOL cancel_pending )
{
    struct threadpool_object *this = impl_from_TP_TIMER( timer );
 ...
 *           TpWaitForWait    (NTDLL.@)
 */
VOID NTAPI TpWaitForWait( TP_WAIT *wait, BOOL cancel_pending )
{
    struct threadpool_object *this = impl_from_TP_WAIT( wait );
 ...
 *           TpWaitForWork    (NTDLL.@)
 */
VOID NTAPI TpWaitForWork( TP_WORK *work, BOOL cancel_pending )
{
    struct threadpool_object *this = impl_from_TP_WORK( work );
