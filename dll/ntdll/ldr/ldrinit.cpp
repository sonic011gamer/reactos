                                 ...
                                 OUT PIMAGE_LOAD_CONFIG_DIRECTORY* OutLoadConfig)
{
    ...
    *CookieNotWritable = FALSE;
    ...
    NTSTATUS Status = RtlImageNtHeaderEx(RTL_IMAGE_NT_HEADER_EX_FLAG_NO_RANGE_CHECK,
                                         ...
                                         0,
                                         &NtHeader);
    DPRINT("RtlImageNtHeaderEx returned with status: %X", Status);
    ...
    if (!LoadConfig)
    {
        DPRINT("LdrImageDirectoryEntryToLoadConfig -> NULL");
        return NULL;
    }
                       ...
                       OUT PIMAGE_LOAD_CONFIG_DIRECTORY* OutLoadConfig)
{
   // LDR_FUNC(BOOLEAN, BaseAddress, SizeOfImage, CookieOverride, Salt, OutLoadConfig)
    ...
    {
        DPRINT1("Windows 8.1/10 PE load config detected");
...
InitSecurityCookie()
{
   // LDR_FUNC_VOID_MANUAL()
    ...
    {
        LARGE_INTEGER Delay;
        Delay.QuadPart = Int32x32To64(30, -10000);
...
LdrpInitializeThread(IN PCONTEXT Context)
{
    ...
    DPRINT1("LdrpInitializeThread() called for {%s} (", &LdrpImageEntry->BaseDllName);
    ...
    if (!NT_SUCCESS(Status))
    {
        DPRINT("Unable to allocate ActivationContextStack");
    }
    ...
    while (TRUE)
    {
        ...
        Timeout.QuadPart = Int32x32To64(300, -10000);
...
LdrShutdownProcess()
{
    ...
    DPRINT("Process %X, %s exiting", Teb->ClientId.UniqueProcess, &LdrpImageEntry->BaseDllName);
...
LdrpApplyProcessPermanentNoExecuteMitigationPolicy()
{
   // LDR_FUNC_VOID_MANUAL()
    ...
    ExecuteOptions.ExecuteOptionsNV = 0;
