...
GetKeyName(HKEY hKey, PUNICODE_STRING KeyName)
{
    ...
    {
        DPRINT1("NtQueryKey returned unexpected Status: 0x%08x\n", Status);
    ...
    {
        DPRINT1("Failed to allocate %lu bytes", InfoLength);
    ...
    if (!NT_SUCCESS(Status))
    {
        ...
        DPRINT1("NtQueryKey failed: 0x%08x\n", Status);
        return RtlNtStatusToDosError(Status);
    }
    ...
    {
        RtlFreeHeap(RtlGetProcessHeap(), 0, NameInformation);
        DPRINT1("RtlDuplicateUnicodeString failed: 0x%08x\n", Status);
        return RtlNtStatusToDosError(Status);
    }
    ...
    _Out_ REGSAM* RegSam)
{
    ...
    if (!NT_SUCCESS(Status))
    {
        DPRINT1("NtQueryObject failed, Status %x08x\n", Status);
        return RtlNtStatusToDosError(Status);
    }
    ...
    _Out_opt_ PFILETIME lpftLastWriteTime)
{
    ...
    if (ErrorCode != ERROR_SUCCESS)
    {
        DPRINT1("Could not query info of key %p (Err: %d)\n", FallbackKey, ErrorCode);
        goto Exit;
    }

    MaxFallbackSubKeyLen++;
    DPRINT("Maxfallbacksubkeylen: %d\n", MaxFallbackSubKeyLen);
    ...
    while (TRUE)
    {
        ...
        {
            DPRINT1("Got error %d while querying for %s on HKCU side.\n", ErrorCode, FallbackSubKeyName);
            goto Exit;
        }
