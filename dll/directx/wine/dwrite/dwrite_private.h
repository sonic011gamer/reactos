...
#ifdef __REACTOS__ /* This is just to fix GCC/Clang AMD64 build */
extern unsigned int opentype_layout_find_script(const struct scriptshaping_cache *cache, DWORD kind, DWORD tag,
        unsigned int *script_index) DECLSPEC_HIDDEN;
extern unsigned int opentype_layout_find_language(const struct scriptshaping_cache *cache, DWORD kind, DWORD tag,
