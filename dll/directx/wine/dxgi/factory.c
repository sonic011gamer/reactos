...
//TODO: FIXME: please jesus
wined3d_swapchain = NULL;
    DPRINT1("IWineDXGIDevice_create_swapchain isn't imported yet\n BREAKING BEFORE IT'S NEEDED\n");
__debugbreak();

    hr = 0;//IWineDXGIDevice_create_swapchain(dxgi_device, &wined3d_desc, FALSE, &wined3d_swapchain);    IWineDXGIDevice_Release(dxgi_device);
    if (FAILED(hr))
    {
...
struct dxgi_factory *unsafe_impl_from_IDXGIFactory1(IDXGIFactory1 *iface)
{
    if (!iface)
        return NULL;
   if(iface->lpVtbl == &dxgi_factory_vtbl){
        DPRINT1("unsafe_impl_from_IDXGIFactory1: Something has gone wrong");
   }
    return CONTAINING_RECORD(iface, struct dxgi_factory, IDXGIFactory1_iface);
}
