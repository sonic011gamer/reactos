    ...
    _In_ PHHIVE RegistryHive)
{
    ...
    {
        DPRINT1("HvpWriteLog(): The sequences DO NOT MATCH (Sequence1 == %lu, Sequence2 == %lu)\n",
     ...
     */
    Size = RegistryHive->DirtyVector.SizeOfBitMap + sizeof(ULONG);
    BitmapSize = ROUND_UP(Size, HSECTOR_SIZE);
    BufferSize = BitmapSize + HV_LOG_HEADER_SIZE;

    /* Now allocate the base header block buffer */
    HeaderBuffer = RegistryHive->Allocate(BufferSize, TRUE, TAG_CM);
    if (!HeaderBuffer)
    {
        DPRINT1("HvpWriteLog(): Couldn't allocate buffer for base header block!\n");
        return FALSE;
    }
    ...
    *((PULONG)Ptr) = HV_LOG_DIRTY_SIGNATURE;
    ...
    if (!Success)
    {
        DPRINT1("HvpWriteLog(): Failed to write the hive header block to log (primary sequence)!\n");
        return FALSE;
    }

    /*
     * We are acknowledged that we have a sane base block
     * and we incremented the primary sequence so flush out
     * the log to the storage immediately. If some shit
     * happens down the road at least the system knows
     * what action must be taken.
     */
    Success = RegistryHive->FileFlush(RegistryHive, HFILE_TYPE_LOG, NULL, 0);
    if (!Success)
    {
        DPRINT1("HvpWriteLog(): Failed to flush the log!\n");
        return FALSE;
    }
    ...
    while (BlockIndex < RegistryHive->Storage[Stable].Length)
    {
        ...
        if (!Success)
        {
            DPRINT1("HvpWriteLog(): Failed to write dirty block to log (block %p, block index %lu)!\n", Block, BlockIndex);
            return FALSE;
        }
    ...
    /*
     * We wrote the body of log with dirty data,
     * do another flush immediately.
     */
    Success = RegistryHive->FileFlush(RegistryHive, HFILE_TYPE_LOG, NULL, 0);
    if (!Success)
    {
    ...
    if (!Success)
    {
        DPRINT1("HvpWriteLog(): Failed to write the log file (secondary sequence)!\n");
        return FALSE;
    }
    ...
    _In_ BOOLEAN OnlyDirty)
{
    ...
    if (RegistryHive->BaseBlock->Sequence1 !=
        RegistryHive->BaseBlock->Sequence2)
    {
        DPRINT1("HvpWriteHive(): The sequences DO NOT MATCH (Sequence1 == %lu, Sequence2 == %lu)\n",
                RegistryHive->BaseBlock->Sequence1, RegistryHive->BaseBlock->Sequence2);
        return FALSE;
    }
    ...
    if (!Success)
    {
        DPRINT1("HvpWriteHive(): Failed to write the base block header to primary hive (primary sequence)!\n");
        return FALSE;
    }

    /*
     * Flush the hive immediately. We want that
     * the updated primary sequence number gets
     * visible physically on disk.
     */
    Success = RegistryHive->FileFlush(RegistryHive, HFILE_TYPE_PRIMARY, NULL, 0);
    if (!Success)
    {
        DPRINT1("HvpWriteHive(): Failed to flush the primary hive!\n");
        return FALSE;
    }
    ...
    while (BlockIndex < RegistryHive->Storage[Stable].Length)
    {
        ...
        if (!Success)
        {
            DPRINT1("HvpWriteHive(): Failed to write hive block to primary hive file (block %p, block index %lu)!\n",
                    Block, BlockIndex);
            return FALSE;
        }
    ...
    if (!Success)
    {
        DPRINT1("HvpWriteHive(): Failed to write the base block header to primary hive (secondary sequence)!\n");
        return FALSE;
    }
    ...
    _In_ PHHIVE RegistryHive)
{
    ...
    /*
     * The hive must be writable and with a
     * valid signature for syncing.
     */
    ASSERT(RegistryHive->ReadOnly == FALSE);
    ...
    {
        DPRINT1("HvSyncHive(): The dirty vector has clean data, nothing to do!\n");
        return TRUE;
    }
    ...
    if (CmpMiniNTBoot)
    {
        DPRINT1("HvSyncHive(): We are sharing hives or in Live CD mode, abort syncing!\n");
        return TRUE;
    }
#endif
    ...
    {
        DPRINT1("HvSyncHive(): The hive is volatile (hive %p)!\n", RegistryHive);
        return TRUE;
    }
    ...
    if (RegistryHive->Log == TRUE)
    {
        ...
        {
            DPRINT1("HvSyncHive(): Failed to write a log whilst syncing the hive!\n");
#if !defined(CMLIB_HOST) && !defined(_BLDR_)
    ...
    {
        DPRINT1("HvSyncHive(): Failed to write the primary hive!\n");
#if !defined(CMLIB_HOST) && !defined(_BLDR_)
        IoSetThreadHardErrorMode(HardErrors);
#endif
        return FALSE;
    ...
    _In_ PHHIVE RegistryHive)
{
    ...
    {
        DPRINT1("HvWriteHive(): Failed to write the hive!\n");
        return FALSE;
    }
