    ...
    _In_ PHHIVE RegistryHive)
{
    ...
    {
        DPRINT1("HvpWriteLog(): The sequences DO NOT MATCH (Sequence1 == 0x%x, Sequence2 == 0x%x)\n",
    ...
    *((PULONG)Ptr) = HV_LOG_DIRTY_SIGNATURE;
    ...
    while (BlockIndex < RegistryHive->Storage[Stable].Length)
    {
        ...
        if (!Success)
        {
            DPRINT1("HvpWriteLog(): Failed to write dirty block to log (block %p, block index 0x%x)!\n", Block, BlockIndex);
            return FALSE;
        }
    ...
    _In_ BOOLEAN OnlyDirty)
{
    ...
    if (RegistryHive->BaseBlock->Sequence1 !=
        RegistryHive->BaseBlock->Sequence2)
    {
        DPRINT1("HvpWriteHive(): The sequences DO NOT MATCH (Sequence1 == 0x%x, Sequence2 == 0x%x)\n",
                RegistryHive->BaseBlock->Sequence1, RegistryHive->BaseBlock->Sequence2);
        return FALSE;
    }
    ...
    while (BlockIndex < RegistryHive->Storage[Stable].Length)
    {
        ...
        if (!Success)
        {
            DPRINT1("HvpWriteHive(): Failed to write hive block to primary hive file (block %p, block index 0x%x)!\n",
                    Block, BlockIndex);
            return FALSE;
        }
