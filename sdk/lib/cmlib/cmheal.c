...
/**
 * @brief
 * Removes the offending subkey from a non root index.
 *
 * @param[in] Hive
 * A pointer to a hive.
 *
 ...
 *
 * @param[in] Index
 * A pointer to a key index of which the offending subkey
 ...
 */
BOOLEAN
CmpRemoveSubKeyNonRoot(
    _In_ PHHIVE Hive,
    _In_ PCM_KEY_NODE KeyNode,
    _In_ PCM_KEY_INDEX Index,
    _In_ HCELL_INDEX TargetKey)
{
    PCM_KEY_FAST_INDEX FastIndex;
    ULONG KeyIndex;
    ...
    /* Loop the index */
    for (KeyIndex = 0; KeyIndex < Index->Count; KeyIndex++)
    {
        /*
         * Check if the main index is a fast
         * leaf or a hash one.
         */
            ...
            (Index->Signature == CM_KEY_HASH_LEAF))
        {
             ...
             * searching?
             */
            if (FastIndex->List[KeyIndex].Cell == TargetKey)
            {
                /*
                ...
                CmpRemoveFastIndexKeyCell(FastIndex, KeyIndex, FALSE);

                /*
                 * Since this fast index actually came from the
                 * actual key index itself, just update its count
                 * rather than that of the fast index.
                 */
                Index->Count--;
                DPRINT1("CmpRemoveSubKeyNonRoot(): The offending key cell has BEEN FOUND in fast index (fast index %p, index %lu)!\n",
                        FastIndex, KeyIndex);
                return TRUE;
            }
        }
        ...
        {
            /*
             * The index is neither of the two. The offending
             * cell must come directly from the index at this
             * point.
             */
            if (Index->List[KeyIndex] == TargetKey)
            {
                /*
                 * We have found it. Now we must
                 ...
                 */
                HvMarkCellDirty(Hive, KeyNode->SubKeyLists[Stable], FALSE);
                CmpRemoveIndexKeyCell(Index, KeyIndex);
                DPRINT1("CmpRemoveSubKeyNonRoot(): The offending key cell has BEEN FOUND in main index (main index %p, index %lu)!\n",
                        Index, KeyIndex);
                return TRUE;
            }
        }
     ...
     * hunt the offending target key cell.
     */
    DPRINT1("CmpRemoveSubKeyNonRoot(): No target key has been found to remove!\n");
    return FALSE;
}
...
CmIsSelfHealEnabled(VOID)
{
    ...
    if (CmSelfHeal || (CmpBootType & HBOOT_TYPE_SELF_HEAL))
        return TRUE;
    ...
    _In_ HCELL_INDEX ParentKey)
{
    ...
    {
        /* Otherwise call the other helper */
        ParentRepaired = CmpRemoveSubKeyNonRoot(Hive,
                                                KeyNode,
                                                KeyIndex,
                                                TargetKey);
    }
