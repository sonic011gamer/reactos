    ...
    _Inout_ PLARGE_INTEGER TimeStamp)
{
    ...
    *HiveBaseBlock = NULL;
    ...
    /*
     * Read the header from the primary hive
     * file. If we fail, then this hive is kaput.
     */
    if (!Result)
    {
        DPRINT1("HvpGetHiveHeader(): Failed to read the base block header from primary hive!\n");
        Hive->Free(BaseBlock, Hive->BaseBlockAlloc);
        return NotHive;
    }
    ...
    _In_ PHHIVE Hive)
{
    ...
    if (!NT_SUCCESS(Status))
    {
        ...
        DPRINT1("HvpComputeHiveSpaceSize(): ZwQueryInformationFile returned 0x%lx\n", Status);
        return HiveSize;
    }
    ...
    _Inout_ PHBASE_BLOCK *BaseBlock)
{
    ...
    if (!Success)
    {
        DPRINT1("HvpRecoverHeaderFromLog(): Failed to read the log file!\n");
        Hive->Free(LogHeader, Hive->BaseBlockAlloc);
        return Fail;
    }

    /* Validate the log header and timestamp */
    ...
    {
        /*
         * The log header or timestamp is invalid. See if
         * self-heal mode is enabled for this system.
         */
    ...
    _In_opt_ PCUNICODE_STRING FileName)
{
    ...
    switch (Result)
    {
        ...
        case RecoverHeader:
        {
            ...
            if (Result2 == Fail)
            {
                DPRINT1("HvLoadHive(): Failed to recover the hive data!\n");
                return STATUS_REGISTRY_CORRUPT;
            }
