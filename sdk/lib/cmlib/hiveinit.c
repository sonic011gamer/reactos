    ...
    _Inout_ PLARGE_INTEGER TimeStamp)
{
    ...
    *HiveBaseBlock = NULL;
    ...
    {
        /*
         * Don't assume the hive is ultiamtely destroyed
        ...
        if (!Result)
        {
            DPRINT1("HvpGetHiveHeader(): Failed to read the first block of the first bin hive (hive too corrupt)!\n");
            Hive->Free(BaseBlock, Hive->BaseBlockAlloc);
            return NotHive;
        }
        ...
        *TimeStamp = BaseBlock->TimeStamp;
        DPRINT1("HvpGetHiveHeader(): The hive is not fully corrupt, the base block needs to be RECOVERED!!!\n");
