 ...
 *
 * @param[in] CheckSequence
 * If set to TRUE the function will also validate
 * the primary and secondary sequences on demand.
 * If set to FALSE the sequences won't be checked.
 *
 * @return
    ...
    _In_ BOOLEAN CheckSequence)
{
        ...
        HvpHiveHeaderChecksum(BaseBlock) != BaseBlock->CheckSum)
    {
        ...
        DPRINT1("    Checksum: 0x%x, expected 0x%x\n",
                HvpHiveHeaderChecksum(BaseBlock), BaseBlock->CheckSum);
    ...
    if (CheckSequence)
    {
        ...
        {
            DPRINT1("Verify Hive Header failed: Sequence 0x%x, expected 0x%x\n",
    ...
    _In_opt_ PCUNICODE_STRING FileName)
{
    ...
    if (ChunkSize < sizeof(HBASE_BLOCK) ||
        !HvpVerifyHiveHeader(ChunkBase, HFILE_TYPE_PRIMARY, TRUE))
    {
...
HvpInitializeFlatHive(
    ...
    _In_ PHBASE_BLOCK ChunkBase)
{
    if (!HvpVerifyHiveHeader(ChunkBase, HFILE_TYPE_PRIMARY, TRUE))
        return STATUS_REGISTRY_CORRUPT;
    ...
    _Inout_ PLARGE_INTEGER TimeStamp)
{
    ...
    *HiveBaseBlock = NULL;
    ...
    if (!BaseBlock)
    {
        DPRINT1("HvpGetHiveHeader(): Failed to allocate an aligned base block buffer!\n");
        return NoMemory;
    }
    ...
    /*
     * Corruption of the base header block of a hive
     * comes in two different ways. Either the base block
     * memory is bogus and we could not be able to read it
     * from the hive file (as in this case) or the base block
     * is valid but with bogus internal data (see the case below)
     * where the base header is validated.
     */
    if (!Result)
    {
        /*
         * We can't read the base block, but that doesn't necessarily
         * mean the whole hive is catastrophically obliterated. Check
         * that the first block of the first bin is OK, this way we
         * can determine if there is still some hope for this hive.
         */
        DPRINT1("HvpGetHiveHeader(): Couldn't be able to read the base block, trying to read the first bin...\n");
        ...
        Result = Hive->FileRead(Hive,
                                ...
                                Hive->Cluster * HSECTOR_SIZE);
        /*
         * If we can't read the first block too then this
         * hive is hopeless.
         */
        if (!Result)
        {
            DPRINT1("HvpGetHiveHeader(): Failed to read the first bin!\n");
            Hive->Free(BaseBlock, Hive->BaseBlockAlloc);
            return NotHive;
        }
        ...
        *TimeStamp = BaseBlock->TimeStamp;
        Hive->Free(BaseBlock, Hive->BaseBlockAlloc);
        DPRINT1("HvpGetHiveHeader(): The hive base header block needs to be RECOVERED!!!\n");
        return RecoverHeader;
    }

    /*
     * This hive has a base block that's not maimed
     * but are the header data valid?
     */
    if (!HvpVerifyHiveHeader(BaseBlock, HFILE_TYPE_PRIMARY, FALSE))
    {
        DPRINT1("HvpGetHiveHeader(): The hive base header block needs to be RECOVERED!!!\n");
        *TimeStamp = BaseBlock->TimeStamp;
        Hive->Free(BaseBlock, Hive->BaseBlockAlloc);
    ...
    /*
     * As we have a perfectly sane header block,
    ...
    *TimeStamp = BaseBlock->TimeStamp;
    ...
    /*
     * The base header block is OK but is the
     * data inside the hive OK?
     */
    if (BaseBlock->Sequence1 != BaseBlock->Sequence2)
    {
        /*
         * A previous attempt of writing the data into
         * the hive has failed so this potentially could
         * leave the hive corrupt. Tell the caller the
         * data needs to be recovered.
         */
        DPRINT1("HvpGetHiveHeader(): The hive data needs to be RECOVERED!!!\n");
        return RecoverData;
    }
 ...
 * the endpoint file size of the associated
 * hive file.
 *
 * @param[in] Hive
    ...
    _In_ PHHIVE Hive)
{
    ...
    /* Get the Cm version of the hive descriptor and get the handle */
    CmHive = (PCMHIVE)Hive;
    HiveHandle = CmHive->FileHandles[HFILE_TYPE_PRIMARY];

    /*
     * Query the end of file point of the physical
     * hive file. We need that information in order
     * to ensure how big the hive actually is.
     */
#if !defined(CMLIB_HOST) && !defined(_BLDR_)
    Status = ZwQueryInformationFile(HiveHandle,
                                    &IoStatusBlock,
                                    &EndOfFileInfo,
    ...
    {
        /* We failed at querying the size of hive file... */
        return HiveSize;
    }

    /* Now compute the hive size */
    HiveSize = EndOfFileInfo.EndOfFile.LowPart - HBLOCK_SIZE;
#endif
    return HiveSize;
}
    ...
    _Inout_ PHBASE_BLOCK *BaseBlock)
{
    ...
    if (!LogHeader)
    {
        DPRINT1("HvpRecoverHeaderFromLog(): Failed to allocate memory for the log header!\n");
        return NoMemory;
    }
    ...
    Success = Hive->FileRead(Hive,
                             ...
                             Hive->Cluster * HSECTOR_SIZE);
    if (!Success ||
        !HvpVerifyHiveHeader(LogHeader, HFILE_TYPE_LOG, TRUE) ||
        TimeStamp->HighPart != LogHeader->TimeStamp.HighPart ||
        TimeStamp->LowPart != LogHeader->TimeStamp.LowPart)
    {
        /*
         * We failed? Looks like the log is corrupt
         * or something. See if self-healing mode
         * is permitted.
         */
        if (!CmIsSelfHealEnabled())
        {
            /* If the DOOM guy could help us... we're hopeless */
            DPRINT1("HvpRecoverHeaderFromLog(): The log couldn't be read and self-healing mode is disabled!\n");
         ...
         */
        HiveSize = HvpComputeHiveSpaceSize(Hive);
        if (HiveSize == 0)
        {
            DPRINT1("HvpRecoverHeaderFromLog(): Failed to query the hive size!\n");
            Hive->Free(LogHeader, Hive->BaseBlockAlloc);
            return Fail;
        }
    ...
    if (!Success)
    {
        DPRINT1("HvpRecoverHeaderFromLog(): Couldn't write the base header to primary hive!\n");
        Hive->Free(LogHeader, Hive->BaseBlockAlloc);
        return Fail;
    }

    /*
     * Give the base header to the caller and check
     * again if we have resuscitated the header.
     * If so then we must trigger the self-heal mode.
     */
    *BaseBlock = LogHeader;
    if (HeaderResuscitated)
    {
        DPRINT1("HvpRecoverHeaderFromLog(): Header has been resuscitated, triggering self-heal mode!\n");
        return SelfHeal;
    }
    ...
    _In_ PHBASE_BLOCK BaseBlock)
{
    ...
    {
        /*
         * We failed? See if self-heal mode is
         * enabled on this system.
         */
        if (!CmIsSelfHealEnabled())
        {
            DPRINT1("HvpRecoverDataFromLog(): The log couldn't be read and self-healing mode is disabled!\n");
            return Fail;
        }
         ...
         */
        DPRINT1("HvpRecoverDataFromLog(): Triggering self-heal mode, DATA LOSS IS IMMINENT!!!\n");
        return SelfHeal;
    }
    ...
    {
        /*
         * Uh oh? It looks like the dirty vector contains
         * garbage we do not want. Usually that would happen
         * when writing to log has been interrupted. Check
         * if self-heal mode is enabled.
         */
        if (!CmIsSelfHealEnabled())
        {
            DPRINT1("HvpRecoverDataFromLog(): The log's dirty vector signature is not genuine!\n");
            return Fail;
        }
    ...
    for (BlockIndex = 0; BlockIndex < StorageLength; BlockIndex++)
    {
        ...
        if (!Success)
        {
            DPRINT1("HvpRecoverDataFromLog(): Failed to read the dirty block (index %lu)\n", BlockIndex);
            return Fail;
        }
        ...
        if (!Success)
        {
            DPRINT1("HvpRecoverDataFromLog(): Failed to write dirty block to hive (index %lu)\n", BlockIndex);
            return Fail;
        }
    ...
    _In_opt_ PCUNICODE_STRING FileName)
{
    ...
    switch (Result)
    {
        ...
        {
            /* Fail */
            DPRINT1("HvLoadHive(): There's no enough memory to get the header!\n");
        ...
        {
            /* Fail */
            DPRINT1("HvLoadHive(): The hive is not an actual registry hive file!\n");
        ...
        {
            /*
             * Check if this hive has a log at hand
             * to begin with. Otherwise we're truly
             * doomed.
             */
            #if (NTDDI_VERSION < NTDDI_VISTA)
            if (!Hive->Log)
            {
                DPRINT1("HvLoadHive(): The hive has no log for data recovery!\n");
                return STATUS_REGISTRY_CORRUPT;
            }
            #endif

            /*
             * The data needs to be recovered so acknowledge
             * that. We will be recovering the data after we
             * read the hive.
             */
            DPRINT1("HvLoadHive(): Attempting to heal the data...\n");
            break;
        }
        ...
        case RecoverHeader:
        {
            ...
            if (!Hive->Log)
            {
                DPRINT1("HvLoadHive(): The hive has no log for header recovery!\n");
                return STATUS_REGISTRY_CORRUPT;
            }
            #endif

            /* The header needs to be recovered so do it */
            DPRINT1("HvLoadHive(): Attempting to heal the header...\n");
            Result2 = HvpRecoverHeaderFromLog(Hive, &TimeStamp, &BaseBlock);
            if (Result2 == NoMemory)
            {
                DPRINT1("HvLoadHive(): There's no enough memory to recover header from log!\n");
                Hive->Free(BaseBlock, Hive->BaseBlockAlloc);
                return STATUS_INSUFFICIENT_RESOURCES;
            }
            ...
            if (Result2 == Fail)
            {
                DPRINT1("HvLoadHive(): Failed to recover the hive header!\n");
                Hive->Free(BaseBlock, Hive->BaseBlockAlloc);
                return STATUS_REGISTRY_CORRUPT;
            }
    ...
    /* Set the boot type */
    BaseBlock->BootType = HeaderSelfHeal ? HBOOT_TYPE_SELF_HEAL : HBOOT_TYPE_REGULAR;
    ...
    {
        Hive->Free(BaseBlock, Hive->BaseBlockAlloc);
        DPRINT1("HvLoadHive(): There's no enough memory to allocate hive data!\n");
        return STATUS_INSUFFICIENT_RESOURCES;
    }
    ...
    Success = Hive->FileRead(Hive,
                             ...
                             HiveData,
                             FileSize);
    if (!Success && (Result != RecoverHeader) && (Result != RecoverData))
    {
        DPRINT1("HvLoadHive(): Failed to read the whole hive!\n");
    ...
    /*
     * Check if registry data recovery needs to
     * be done. RecoverHeader also implies data
     * recovery because if the header is corrupt
     * then whatever comes after the base block is
     * also corrupt.
     */
    if ((Result == RecoverHeader) || (Result == RecoverData))
    {
        Result = HvpRecoverDataFromLog(Hive, BaseBlock);
        if (Result == Fail)
        {
            DPRINT1("HvLoadHive(): Failed to recover data from log!\n");
            Hive->Free(HiveData, FileSize);
            Hive->Free(BaseBlock, Hive->BaseBlockAlloc);
            return STATUS_REGISTRY_CORRUPT;
        }

        /* Did we trigger the self-heal? */
        if (Result == SelfHeal)
        {
        ...
        /*
         * Since we applied recover data to the hive
         * we must read it again. If we fail, the
         * registry is fucked.
         */
        Success = Hive->FileRead(Hive,
                                 HFILE_TYPE_PRIMARY,
        ...
        if (!Success)
        {
            DPRINT1("HvLoadHive(): Data recovery applied but failed to read the whole hive!\n");
            Hive->Free(HiveData, FileSize);
            Hive->Free(BaseBlock, Hive->BaseBlockAlloc);
            return STATUS_NOT_REGISTRY_FILE;
        }
    ...
    if (!NT_SUCCESS(Status))
    {
        DPRINT1("HvLoadHive(): Failed to initialize hive from memory!\n");
        Hive->Free(HiveData, FileSize);
    }

    /*
     * Check again if we applied header or data
     * recovery. Acknowledge the caller we recovered
 ...
 *                  in volatile memory of the system, aka the RAM.
 *
    ...
    switch (OperationType)
    {
        ...
        case HINIT_FILE:
        {
                ...
                (Status != STATUS_REGISTRY_RECOVERED))
            {
                ...
                DPRINT1("HvInitialize: Registry hive couldn't be initialized, it's corrupt (hive %p)!\n", Hive);
                return Status;
            }

            /*
             * Check if we have recovered this poor little
             * innocent hive. In case we did we are responsible
             * to flush the primnary hive back to backing storage
             * as we have healthy and recovered data.
             */
            if (Status == STATUS_REGISTRY_RECOVERED)
            {
                if (!HvSyncHiveFromRecover(Hive))
                {
                    /* We failed, oh shit mama I wanna go home.... */
                    DPRINT1("HvInitialize: Faile to write healthy data back to hive!\n");
        ...
        case HINIT_MEMORY_INPLACE:
        {
            ...
            // break;
            DPRINT1("HvInitialize: HINIT_MEMORY_INPLACE is UNIMPLEMENTED!\n");
            return STATUS_NOT_IMPLEMENTED;
        }

        case HINIT_MAPFILE:
        {
            DPRINT1("HvInitialize: HINIT_MAPFILE is UNIMPLEMENTED!\n");
            return STATUS_NOT_IMPLEMENTED;
        }

        default:
        {
            DPRINT1("HvInitialize: Invalid operation type (OperationType = %lu)!\n", OperationType);
