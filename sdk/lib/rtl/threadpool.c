...
#include "../../include/reactos/wine/list.h"
...
static RTL_CRITICAL_SECTION_DEBUG critsect_compl_debug =
{
    ...
    { &critsect_compl_debug.ProcessLocksList, &critsect_compl_debug.ProcessLocksList },
      0, 0,  (ULONG_PTR)(__FILE__ ": threadpool_compl_cs") 
};
...
static RTL_CRITICAL_SECTION_DEBUG timerqueue_debug =
{
    ...
    { &timerqueue_debug.ProcessLocksList, &timerqueue_debug.ProcessLocksList },
      0, 0,  (ULONG_PTR)(__FILE__ ": timerqueue.cs") 
};
...
static RTL_CRITICAL_SECTION_DEBUG waitqueue_debug =
{
    ...
    { &waitqueue_debug.ProcessLocksList, &waitqueue_debug.ProcessLocksList },
      0, 0,  (ULONG_PTR)(__FILE__ ": waitqueue.cs") 
};
...
static void CALLBACK timerqueue_thread_proc( void *param )
{
    ...
    DPRINT1( "starting timer queue thread\n" );
    ...
    DPRINT1( "terminating timer queue thread\n" );
    RtlExitUserThread( 0 );
}
...
static void CALLBACK waitqueue_thread_proc( void *param )
{
    ...
    DPRINT1( "starting wait queue thread\n" );
    ...
    DPRINT1( "terminating wait queue thread\n" );
...
static NTSTATUS tp_threadpool_alloc( struct threadpool **out )
{
    ...
    pool->cs.DebugInfo->Flags = (DWORD_PTR)(__FILE__ ": threadpool.cs");
    ...
    DPRINT1( "allocated threadpool %p\n", pool );
...
static BOOL tp_threadpool_release( struct threadpool *pool )
{
    ...
    DPRINT1( "destroying threadpool %p\n", pool );

    assert( pool->shutdown );
    assert( !pool->objcount );
    assert( list_empty( &pool->pool ) );

    pool->cs.DebugInfo->Flags = 0;
...
static NTSTATUS tp_group_alloc( struct threadpool_group **out )
{
    ...
    group->cs.DebugInfo->Flags = (DWORD_PTR)(__FILE__ ": threadpool_group.cs");

    list_init( &group->members );

    DPRINT1( "allocated group %p\n", group );
...
static BOOL tp_group_release( struct threadpool_group *group )
{
    ...
    DPRINT1( "destroying group %p\n", group );

    assert( group->shutdown );
    assert( list_empty( &group->members ) );

    group->cs.DebugInfo->Flags = 0;
                                  ...
                                  PVOID userdata, TP_CALLBACK_ENVIRON *environment )
{
    ...
    DPRINT1( "allocated object %p of type %u\n", object, object->type );
...
static BOOL tp_object_release( struct threadpool_object *object )
{
    ...
    DPRINT1( "destroying object %p of type %u\n", object, object->type );
...
static void CALLBACK threadpool_worker_proc( void *param )
{
    ...
    DPRINT1( "starting worker thread for pool %p\n", pool );
    ...
    for (;;)
    {
        while ((ptr = list_head( &pool->pool )))
        {
            ...
            switch (object->type)
            {
                ...
                {
                    DPRINT1( "executing simple callback %p(%p, %p)\n",
                           object->u.simple.callback, callback_instance, object->userdata );
                    object->u.simple.callback( callback_instance, object->userdata );
                    DPRINT1( "callback %p returned\n", object->u.simple.callback );
                    break;
                }

                case TP_OBJECT_TYPE_WORK:
                {
                    DPRINT1( "executing work callback %p(%p, %p, %p)\n",
                ...
                {
                    DPRINT1( "executing timer callback %p(%p, %p, %p)\n",
                ...
                {
                    DPRINT1( "executing wait callback %p(%p, %p, %p, %u)\n",
            ...
            {
                DPRINT1( "executing finalization callback %p(%p, %p)\n",
    ...
    DPRINT1( "terminating worker thread for pool %p\n", pool );
    tp_threadpool_release( pool );
    RtlExitUserThread( 0 );
}
...
VOID WINAPI TpReleaseCleanupGroupMembers( TP_CLEANUP_GROUP *group, BOOL cancel_pending, PVOID userdata )
{
    ...
    LIST_FOR_EACH_ENTRY_SAFE( object, next, &members, struct threadpool_object, group_entry )
    {
        ...
        if (!object->shutdown)
        {
            ...
            {
                DPRINT1( "executing group cancel callback %p(%p, %p)\n",
