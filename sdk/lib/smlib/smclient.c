...
/**
 * @brief
 * Connect to the SM API port and register a session "begin" port
 * (Sb), or to issue API requests to SmApiPort.
 *
 ...
 * created in the system name space.
 *
 * @param[in]   SbApiPort
 * LPC port handle (checked, but not used: the
 ...
 *
 * @param[out]  SmApiPort
 * Pointer to a HANDLE, which will be filled with a
 * valid client-side LPC communication port.
 *
 * @return
...
/**
 * @brief
 * This function is called by an environment subsystem server to tell
 * the SM it has terminated the session it managed.
 *
 * @param[in]   SmApiPort
 * Port handle returned by SmConnectToSm().
 *
    ...
    _Out_ PHANDLE pInitialCommandProcessId)
{
    ...
    if (CommandLine)
    {
        if (CommandLine->Length >= sizeof(SmApiMsg.u.StartCsr.Buffer))
            return STATUS_INVALID_PARAMETER;

        SmApiMsg.u.StartCsr.Length = CommandLine->Length;
        RtlCopyMemory(SmApiMsg.u.StartCsr.Buffer,
                      CommandLine->Buffer,
                      CommandLine->Length);
    }
    else
    {
        SmApiMsg.u.StartCsr.Length = 0;
    }
    SmApiMsg.u.StartCsr.Buffer[SmApiMsg.u.StartCsr.Length / sizeof(WCHAR)] = UNICODE_NULL;
