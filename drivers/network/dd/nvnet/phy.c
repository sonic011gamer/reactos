    ...
    _In_ BOOLEAN DisableCrossoverDetection)
{
    ...
    if (Adapter->Flags & DEV_NEED_PHY_INIT_FIX)
    {
        MiiRead(Adapter, Adapter->PhyAddress, PHY_REALTEK_INIT_REG6, &MiiRegister);
    ...
    _In_ PNVNET_ADAPTER Adapter)
{
    ...
    if (Adapter->PhyModel == PHY_MODEL_MARVELL_E3016)
    {
        ...
        {
            NDIS_DbgPrint(MIN_TRACE, ("PHY write to errata reg failed\n"));
    ...
    if (Adapter->PhyOui == PHY_OUI_REALTEK)
    {
            ...
            Adapter->PhyRevision == PHY_REV_REALTEK_8211B)
        {
            ...
            {
                NDIS_DbgPrint(MIN_TRACE, ("Realtek 8211b init failed\n"));
                return NDIS_STATUS_FAILURE;
            }
        }
                 ...
                 Adapter->PhyRevision == PHY_REV_REALTEK_8211C)
        {
            ...
            {
                NDIS_DbgPrint(MIN_TRACE, ("Realtek 8211c init failed\n"));
                return NDIS_STATUS_FAILURE;
            }
        }
        else if (Adapter->PhyModel == PHY_MODEL_REALTEK_8201)
        {
            ...
            {
                NDIS_DbgPrint(MIN_TRACE, ("Realtek 8201 init failed\n"));
                return NDIS_STATUS_FAILURE;
            }
        }
    }
    ...
    {
        NDIS_DbgPrint(MIN_TRACE, ("PHY write to advertise failed\n"));
        return NDIS_STATUS_FAILURE;
    }
    ...
    if (MiiStatus & PHY_GIGABIT)
    {
        ...
        {
            NDIS_DbgPrint(MIN_TRACE, ("PHY init failed\n"));
            return NDIS_STATUS_FAILURE;
        }
    }
    else
    {
        Adapter->GigabitPhy = FALSE;
    }
    ...
    else
    {
        ...
        {
            NDIS_DbgPrint(MIN_TRACE, ("PHY reset failed\n"));
            return NDIS_STATUS_FAILURE;
        }
    }
    ...
    if (Adapter->PhyOui == PHY_OUI_CICADA)
    {
        ...
        {
            NDIS_DbgPrint(MIN_TRACE, ("Cicada Semiconductor init failed\n"));
            return NDIS_STATUS_FAILURE;
        }
    }
    else if (Adapter->PhyOui == PHY_OUI_VITESSE)
    {
        ...
        {
            NDIS_DbgPrint(MIN_TRACE, ("Vitesse Semiconductor init failed\n"));
            return NDIS_STATUS_FAILURE;
        }
    }
    ...
    _Inout_ PNVNET_ADAPTER Adapter)
{
    ...
    {
        ULONG PhyAddress = Phy & 0x1F; /* 1, 2, , 31, 0 */
...
CODE_SEG("PAGE")
BOOLEAN
SidebandUnitAcquireHwSemaphore(
    _Inout_ PNVNET_ADAPTER Adapter)
{
    ULONG i;
    ...
    for (i = 0; i < 2; ++i)
    {
        ...
        {
            Adapter->UnitSemaphoreAcquired = TRUE;
            return TRUE;
        }
...
CODE_SEG("PAGE")
VOID
SidebandUnitReleaseHwSemaphore(
    _Inout_ PNVNET_ADAPTER Adapter)
{
    PAGED_CODE();
    ...
    _Out_ PBOOLEAN FullDuplex)
{
    ...
    *MiiLinkPartnerAbility = 0;
    ...
    if (Adapter->ForceSpeedAndDuplex)
    {
        if (Adapter->UserLinkSpeed == 100)
        {
        ...
        *FullDuplex = Adapter->ForceFullDuplex;
        return TRUE;
    }
...
NvNetUpdateLinkSpeed(
    _In_ PNVNET_ADAPTER Adapter)
{
    ULONG MiiAdvertise, MiiLinkPartnerAbility, MiiExpansion;
    ULONG LinkSpeed, PhyRegister, TxDeferral, PauseFlags, TxRxFlags;
    ...
    if (Adapter->GigabitPhy)
    {
        ...
        if ((LinkSpeed == NVREG_LINKSPEED_10) || (LinkSpeed == NVREG_LINKSPEED_100))
        {
            PhyRegister |= NVREG_SLOTTIME_10_100_FULL;
        }
        else if (LinkSpeed == NVREG_LINKSPEED_1000)
        {
            PhyRegister |= NVREG_SLOTTIME_1000_FULL;
        }
    ...
    if (LinkSpeed == NVREG_LINKSPEED_100)
        PhyRegister |= PHY_100;
    else if (LinkSpeed == NVREG_LINKSPEED_1000)
    ...
    {
        if (LinkSpeed == NVREG_LINKSPEED_1000)
        {
            TxDeferral = NVREG_TX_DEFERRAL_RGMII_1000;
        }
        else
        {
            if (!(MiiExpansion & MII_EXP_LP_AUTONEG) && !FullDuplex &&
                (Adapter->Flags & DEV_HAS_COLLISION_FIX))
            {
    ...
    NV_WRITE(Adapter, NvRegMisc1, NVREG_MISC1_FORCE | (FullDuplex ? 0 : NVREG_MISC1_HD));
    NV_WRITE(Adapter, NvRegLinkSpeed, LinkSpeed | NVREG_LINKSPEED_FORCE);
    ...
    if (FullDuplex)
    {
        if (!Adapter->ForceSpeedAndDuplex && (Adapter->PauseFlags & NV_PAUSEFRAME_AUTONEG))
        {
    ...
    _In_ PNVNET_ADAPTER Adapter)
{
    ...
    if (Adapter->Flags & DEV_HAS_POWER_CNTRL)
    {
        ...
        if ((Adapter->Flags & DEV_NEED_LOW_POWER_FIX) && Adapter->RevisionId >= 0xA3)
        {
    ...
    if (Adapter->Flags & DEV_HAS_MGMT_UNIT)
    {
        ULONG UnitVersion;
        ...
        if ((NV_READ(Adapter, NvRegTransmitterControl) & NVREG_XMITCTL_MGMT_ST) &&
            ...
            SidebandUnitAcquireHwSemaphore(Adapter) &&
            SidebandUnitGetVersion(Adapter, &UnitVersion))
        {
            if (UnitVersion > 0)
            {
                Adapter->MacInUse =
                    !!(NV_READ(Adapter, NvRegMgmtUnitControl) & NVREG_MGMTUNITCONTROL_INUSE);
            }
            else
            {
                Adapter->MacInUse = TRUE;
            }
    ...
    {
        NDIS_DbgPrint(MIN_TRACE, ("Could not find a valid PHY\n"));
        goto Failure;
    }
