    ...
    _In_ PNVNET_ADAPTER Adapter)
{
    ...
    {
        for (Counter = NvRegTxUnicast; Counter <= NvRegTxBroadcast; Counter += sizeof(ULONG))
        {
            NV_READ(Adapter, Counter);
        }
    }
    ...
    _In_ PNVNET_ADAPTER Adapter)
{
    ...
    if ((NV_READ(Adapter, NvRegReceiverControl) & NVREG_RCVCTL_START) && !Adapter->MacInUse)
    {
    ...
    _In_ ULONG PauseFlags)
{
    ...
    if (Adapter->PauseFlags & NV_PAUSEFRAME_TX_CAPABLE)
    {
        ...
        if (PauseFlags & NV_PAUSEFRAME_TX_ENABLE)
        {
            ...
            if (Adapter->Flags & DEV_HAS_PAUSEFRAME_TX_V2)
                PauseEnable = NVREG_TX_PAUSEFRAME_ENABLE_V2;
            if (Adapter->Flags & DEV_HAS_PAUSEFRAME_TX_V3)
            {
    ...
    _In_ BOOLEAN Gate)
{
    ...
    if (!Adapter->MacInUse && (Adapter->Flags & DEV_HAS_POWER_CNTRL))
    {
    ...
    _In_ PNVNET_ADAPTER Adapter)
{
    ...
    i = NV_READ(Adapter, NvRegPowerState);
    if (!(i & NVREG_POWERSTATE_POWEREDUP))
    {
        NV_WRITE(Adapter, NvRegPowerState, i | NVREG_POWERSTATE_POWEREDUP);
    }
    ...
    _Out_writes_bytes_all_(ETH_LENGTH_OF_ADDRESS) PUCHAR MacAddress)
{
    ...
    if (Adapter->Flags & DEV_HAS_CORRECT_MACADDR)
    {
    ...
    else
    {
         ...
         * so that it is not reversed again on a subsequent MiniportInitialize().
         */
    ...
    _Inout_ PNVNET_ADAPTER Adapter)
{
    ...
    switch (Adapter->DeviceId)
    {
        ...
        case 0x0066: /* nForce2 */
        case 0x00D6: /* nForce2 */
            Adapter->Flags = DEV_NEED_TIMERIRQ | DEV_NEED_LINKTIMER;
            break;
        ...
        case 0x0037: /* MCP04 */
        case 0x0038: /* MCP04 */
            Adapter->Flags = DEV_NEED_LINKTIMER | DEV_HAS_LARGEDESC | DEV_HAS_CHECKSUM |
                             DEV_HAS_HIGH_DMA | DEV_HAS_STATISTICS_V1 | DEV_NEED_TX_LIMIT;
            break;

        case 0x0268: /* MCP51 */
        case 0x0269: /* MCP51 */
            Adapter->Flags = DEV_NEED_LINKTIMER | DEV_HAS_HIGH_DMA | DEV_HAS_POWER_CNTRL |
        ...
        case 0x0373: /* MCP55 */
            Adapter->Flags = DEV_NEED_LINKTIMER | DEV_HAS_LARGEDESC | DEV_HAS_CHECKSUM |
                             ...
                             DEV_HAS_STATISTICS_V1 | DEV_HAS_STATISTICS_V2 | DEV_HAS_TEST_EXTENDED |
                             DEV_HAS_MGMT_UNIT | DEV_NEED_TX_LIMIT | DEV_NEED_MSI_FIX;
            break;
        ...
        case 0x0452: /* MCP65 */
        case 0x0453: /* MCP65 */
            Adapter->Flags = DEV_NEED_LINKTIMER | DEV_HAS_LARGEDESC | DEV_HAS_HIGH_DMA |
                             DEV_HAS_POWER_CNTRL | DEV_HAS_MSI | DEV_HAS_PAUSEFRAME_TX_V1 |
                             DEV_HAS_STATISTICS_V1 | DEV_HAS_STATISTICS_V2 | DEV_HAS_TEST_EXTENDED |
                             DEV_HAS_MGMT_UNIT | DEV_HAS_CORRECT_MACADDR | DEV_NEED_TX_LIMIT |
        ...
        case 0x07DF: /* MCP73 */
            Adapter->Flags = DEV_NEED_LINKTIMER | DEV_HAS_HIGH_DMA | DEV_HAS_POWER_CNTRL |
                             DEV_HAS_MSI | DEV_HAS_PAUSEFRAME_TX_V1 | DEV_HAS_STATISTICS_V1 |
                             DEV_HAS_STATISTICS_V2 | DEV_HAS_TEST_EXTENDED | DEV_HAS_MGMT_UNIT |
                             DEV_HAS_CORRECT_MACADDR | DEV_HAS_COLLISION_FIX | DEV_HAS_GEAR_MODE |
                             DEV_NEED_MSI_FIX;
            break;
        ...
        case 0x0762: /* MCP77 */
        case 0x0763: /* MCP77 */
            Adapter->Flags = DEV_NEED_LINKTIMER | DEV_HAS_CHECKSUM | DEV_HAS_HIGH_DMA |
                             DEV_HAS_MSI | DEV_HAS_POWER_CNTRL | DEV_HAS_PAUSEFRAME_TX_V2 |
                             DEV_HAS_STATISTICS_V1 | DEV_HAS_STATISTICS_V2 | DEV_HAS_STATISTICS_V3 |
    ...
    if (!(Adapter->Flags & DEV_HAS_CHECKSUM))
    {
        Adapter->ChecksumOffload = FALSE;
    }
    if (!(Adapter->Flags & DEV_HAS_VLAN))
    {
        Adapter->VlanTaggingSupported = FALSE;
        Adapter->PrioritySupported = FALSE;
    }
    if ((Adapter->Flags & DEV_NEED_TIMERIRQ) &&
        (Adapter->OptimizationMode == NV_OPTIMIZATION_MODE_DYNAMIC))
    {
        Adapter->OptimizationMode = NV_OPTIMIZATION_MODE_THROUGHPUT;
    }
    if (!(Adapter->Flags & DEV_HAS_TX_PAUSEFRAME))
    {
    ...
    /* HACK: Disable some NIC features, we don't support these yet */
    Adapter->VlanTaggingSupported = FALSE;
    Adapter->PrioritySupported = FALSE;
    ...
    /*
     * Limit the number of outstanding transmit packets to 16 for HW bug.
     * No need to modify the number of TX descriptors to allocate,
     * the bug doesn't affect S/G segments.
     */
    if (Adapter->Flags & (DEV_NEED_TX_LIMIT | DEV_NEED_TX_LIMIT2))
    {
        Adapter->LimitTx = TRUE;

        if ((Adapter->Flags & DEV_NEED_TX_LIMIT2) && Adapter->RevisionId >= 0xA2)
        {
            Adapter->LimitTx = FALSE;
        }
    }
    if (Adapter->LimitTx)
    {
