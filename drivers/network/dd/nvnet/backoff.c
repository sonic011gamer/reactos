...
static const ULONG MainSeedSet[BACKOFF_SEEDSET_ROWS][BACKOFF_SEEDSET_LFSRS] =
{
...
static const ULONG GearSeedSet[BACKOFF_SEEDSET_ROWS][BACKOFF_SEEDSET_LFSRS] =
{
    ...
    _In_ PNVNET_ADAPTER Adapter)
{
    ...
    RestartTransmitter = !!(NV_READ(Adapter, NvRegTransmitterControl) & NVREG_XMITCTL_START);
    ...
    _In_ PNVNET_ADAPTER Adapter)
{
    ...
    for (i = 1; i <= BACKOFF_SEEDSET_LFSRS; ++i)
    {
        ...
        Temp |= MainSeedSet[SeedSet][i - 1] & NVREG_BKOFFCTRL_SEED_MASK;
        Temp |= ((GearSeedSet[SeedSet][i - 1] & NVREG_BKOFFCTRL_SEED_MASK) << NVREG_BKOFFCTRL_GEAR);
        NV_WRITE(Adapter, NvRegBackOffControl, Temp);
    }
}
